...
static void setParticlesFromMotions(RigidObjectVector *rov, cudaStream_t stream, MPI_Comm comm)
{
    ...
    integrator.execute(rov, stream, comm);
}
...
void RigidIC::exec(const MPI_Comm& comm, ParticleVector *pv, cudaStream_t stream)
{
    ...
    setParticlesFromMotions(rov, stream, comm);
