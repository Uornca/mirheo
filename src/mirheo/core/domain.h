...
struct DomainInfo
{
    ...
    double bufferSize;   ///< size of the x coordinate buffer
    ...
    {
    	if (globalStart.x == 0) {
            return ((xg.x == -200) || ((globalStart.x <= xg.x) && (xg.x < (globalStart.x + localSize.x))))
                && (globalStart.y <= xg.y) && (xg.y < (globalStart.y + localSize.y))
                && (globalStart.z <= xg.z) && (xg.z < (globalStart.z + localSize.z));
    	} else if (globalStart.x + localSize.x == globalSize.x) {
            return ((xg.x == 200) || ((globalStart.x <= xg.x) && (xg.x < (globalStart.x + localSize.x))))
                && (globalStart.y <= xg.y) && (xg.y < (globalStart.y + localSize.y))
                && (globalStart.z <= xg.z) && (xg.z < (globalStart.z + localSize.z));
        }
 ...
 */
DomainInfo createDomainInfo(MPI_Comm cartComm, real3 globalSize, double bufferSize);
